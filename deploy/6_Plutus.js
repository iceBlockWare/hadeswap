...
module.exports = async function ({ ethers, deployments, getNamedAccounts }) {
    ...
    const { address } = await deploy("Plutus", {
        from: deployer,
        args: [soul.address, dev, "1000000000000000000", "100000", "100"],
        log: true,
    ...
    if (await soul.owner() !== address) {
        ...
        await (await soul.transferOwnership (address)).wait()
        // await (await plutus.claimOwnership (soul.address)).wait()
    }

    if (await plutus.owner() !== dev) {
        // Transfer ownership of Plutus to dev
        console.log("Transfer ownership of Plutus to dev")
        await (await plutus.transferOwnership(dev)).wait()
